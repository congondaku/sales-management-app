...
const CreateSalesPersonModal = ({ onClose, onSuccess }) => {
  const { token } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const validation = validateSalesPersonForm(formData);
      ...
      // Appel à l'API
      const response = await salesService.createSalesPerson(formData);
      ...
      if (response.success) {
        ...
        if (response.validationErrors) {
          ...
          setErrors({ general: [response.message || 'Erreur lors de la création'] });
        }
      }
    } catch (error) {
  ...
  return (
    <FormModal
      ...
      submitText="Créer le Commercial"
      cancelText="Annuler"
      loading={loading}
      size="4xl"
    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Mot de passe (sera changé à la première connexion)"
                disabled={loading}
              />
              {errors.password && (
