...
import {
  ...
  Smartphone, X, Check, AlertCircle, Zap,
} from 'lucide-react';
...
// Vodacom removed — same network as M-Pesa in DRC
const OPERATORS = [
...
const METHOD_LABEL = {
  ...
  airtel:         'Airtel Money',
  orange:         'Orange Money',
  vodacom:        'M-Pesa',       // treat legacy vodacom as mpesa label
  card:           'Carte',
  stripe:         'Stripe',
...
const WithdrawModal = ({ onClose, onSuccess, currentBalanceUSD }) => {
  ...
  const submit = async () => {
    ...
    try {
      const res = await apiClient.post('/finance/withdraw', {
        ...form,
        ...
        <div className="p-5 space-y-4">
          ...
          {result ? (
            <div className="text-center py-6">
              ...
              <button
                onClick={onClose}
                className="mt-5 bg-emerald-600 text-white px-8 py-2.5 rounded-xl text-sm font-medium hover:bg-emerald-700 transition-colors"
              >
                Fermer
              </button>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number" value={form.amount} onChange={set('amount')} min="1"
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {OPERATORS.map(op => (
                    <button
                      key={op.key}
                      onClick={() => setForm(f => ({ ...f, provider: op.key }))}
                      className={`py-2.5 rounded-xl text-sm font-semibold border transition-all ${
                        form.provider === op.key
                          ? 'bg-emerald-600 text-white border-emerald-600 shadow-sm'
                          : 'border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-emerald-400 dark:hover:border-emerald-500'
                      }`}
                    >
                      {op.label}
                    </button>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-black -400 mt-0.5">
      ...
      {error && (
        <div className="flex items-center gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-4">
          ...
          <button onClick={loadData} className="text-sm text-red-600 dark:text-red-400 underline font-medium">Réessayer</button>
        </div>
      )}
        ...
        {[
          {
            ...
            icon:  Wallet,
            color: 'emerald',
            note:  'Revenus depuis le lancement',
          },
          {
          ...
          return (
            <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className={`${colors[card.color]} p-2.5 rounded-xl`}>
                  <Icon className="h-5 w-5 text-white" />
                </div>
                {/* "Current balance" badge on first card */}
                {i === 0 && !loading && (
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
          ...
          {loading ? (
            <div className="space-y-3">
              {[1,2,3,4].map(k => (
                <div key={k} className="h-[72px] bg-gray-100 dark:bg-gray-700 rounded-xl animate-pulse" />
              ))}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border transition-all ${
                      CAT_BG[cat.key] || 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600'
                    } ${sel ? 'ring-2 ring-emerald-500 ring-offset-1 dark:ring-offset-gray-800' : 'hover:opacity-90'}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Icon className={`h-4 w-4 flex-shrink-0 ${CAT_TEXT[cat.key] || 'text-gray-500'}`} />
                        <div>
                          <p className={`text-sm font-semibold ${CAT_TEXT[cat.key] || 'text-gray-700 dark:text-gray-300'}`}>
                            {cat.label}
                          </p>
                    ...
                    <div className="mt-2.5 w-full bg-white/60 dark:bg-gray-600/40 rounded-full h-1.5">
                      <div
                        className="h-1.5 rounded-full bg-current opacity-60 transition-all duration-500"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </button>
            ...
            <div className="mt-4 space-y-2 border-t border-gray-100 dark:border-gray-700 pt-4">
              {data.monthlyTrend.slice(-4).reverse().map((m, i) => (
                <div key={i} className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-semibold ${i === 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-700 dark:text-gray-300'}`}>
                      {usd(m.totalUSD)}
                    </span>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          ...
          {tx && (
            <span className="text-xs text-gray-400 dark:text-gray-500">
              {tx.total.toLocaleString('fr')} résultat{tx.total !== 1 ? 's' : ''}
            </span>
          )}
        </div>
        ...
        {loading ? (
          <div className="p-5 space-y-3">
            {[1,2,3,4,5].map(k => (
              <div key={k} className="h-14 bg-gray-100 dark:bg-gray-700 rounded-xl animate-pulse" />
            ))}
          </div>
        ) : !tx?.data?.length ? (
                  ...
                  <tr key={t._id} className="hover:bg-gray-50/60 dark:hover:bg-gray-700/20 transition-colors">
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap text-right">
                      <p className="text-sm font-bold text-gray-900 dark:text-white">
                        {t.currency === 'USD' ? usd(t.amount) : cdf(t.amount)}
                      </p>
        ...
        {tx && tx.totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3.5 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setPage(p => p + 1)} disabled={page >= tx.totalPages}
                className="p-1.5 rounded-lg border border-gray-300 dark:border-gray-600 disabled:opacity-40 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              >
