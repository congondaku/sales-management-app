...
export const Last10 = ({ data, title = "🔴 Live Activity (Last 10 minutes)", device }) => {
  const chartData = Array.isArray(data) && data.length > 0
    ? data
    : [{ label: "No data", value: 0, color: "bg-slate-600" }];

  const deviceData = Array.isArray(device) && device.length > 0
    ? device
    : [{ label: "No device data", value: 0, color: "bg-slate-600" }];

  return (
    <section className="bg-slate-900 rounded-lg px-4 py-8 mb-8">
      {/* Main breakdown */}
      <div className="mx-auto grid max-w-4xl grid-cols-1 gap-2 md:grid-cols-[1fr_400px] md:gap-12 text-white">
      ...
      {/* Device breakdown */}
      {/* <div className="mx-auto grid max-w-4xl grid-cols-1 gap-2 md:grid-cols-[1fr_400px] md:gap-12 text-white">
        <Legend data={deviceData} title={`${title} — by Device`} />
        <Bars data={deviceData} />
      </div> */}
    </section>
  );
};
    ...
    <div className="col-span-1 py-12">
      ...
      <div className="mb-6 space-y-2">
        {data.map((item, index) => {
          ...
          return (
            <div key={index} className={`w-full rounded-md ${item.color || "bg-slate-600"} p-3 text-white`}>
...
const Bars = ({ data = [] }) => {
  ...
  return (
    ...
    >
      {data.map((item, index) => {
        const val = Number(item?.value || 0);
        const height = ((val / maxValue) * 100).toFixed(2);
        const percentage = totalValue > 0 ? ((val / totalValue) * 100).toFixed(1) : 0;
          ...
          <div key={index} className="col-span-1">
            <div className="relative flex h-full w-full items-end overflow-hidden rounded-2xl bg-gradient-to-b from-slate-700 to-slate-800">
              <motion.span
                ...
                className={`relative z-0 w-full ${item?.color || "bg-slate-600"}`}
              ...
              <span className="absolute bottom-0 left-[50%] mt-2 inline-block w-full -translate-x-[50%] p-2 text-center text-sm text-slate-50">
                <b className="block truncate px-1">{item?.label ?? "Unknown"}</b>
                <span className="block text-xs text-slate-200 mt-1">{val.toLocaleString()}</span>
