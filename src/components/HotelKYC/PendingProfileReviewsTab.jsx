...
import {
  ...
  Bell, ArrowLeft, User, MapPin, Check,
} from 'lucide-react';
...
// ─── What-changed label parser ────────────────────────────────
// pendingProfileReviewReason is a comma-separated list of field keys
const CHANGE_LABELS = {
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
      <div className="flex items-center gap-2">
        ...
        {doc.uploadedAt && (
          <span className="text-xs text-gray-400">· {fmt(doc.uploadedAt)}</span>
        )}
      </div>
      <div className="flex items-center gap-2">
        <a
...
// ─── Photo grid viewer ────────────────────────────────────────
const PhotoGrid = ({ urls, label }) => {
  const [lightbox, setLightbox] = useState(null);
  if (!urls || urls.length === 0) return null;
  return (
    <div>
      <p className="text-xs font-semibold text-gray-500 mb-2">
        {label} ({urls.length} photo{urls.length !== 1 ? 's' : ''})
      </p>
      ...
      {lightbox && (
        ...
        >
          <button
            className="absolute top-4 right-4 p-2 bg-white/20 hover:bg-white/40 rounded-xl text-white"
            onClick={() => setLightbox(null)}
          >
            <X className="h-5 w-5" />
          </button>
    ...
    <div className="space-y-5">
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {changes.map(c => (
              <span
                key={c}
                className="inline-flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-800 text-xs font-semibold rounded-full border border-orange-200"
              >
                {CHANGE_LABELS[c] || c}
              </span>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          ...
          {/* KYC docs */}
          {(hasDocChanges || account.kyc?.rccm?.url || account.kyc?.nationalId?.url || account.kyc?.nif?.url) && (
            <div className="bg-white rounded-2xl border border-gray-200 p-5">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                Documents KYC
                {hasDocChanges && (
                  <span className="ml-2 normal-case text-orange-600 font-semibold">
                    · modifiés
                  </span>
                )}
              </p>
              ...
              <DocViewer label="NIF (optionnel)"      doc={account.kyc?.nif}        />
            </div>
          )}
        </div>
        ...
        {/* Right — hotel photos */}
        <div className="space-y-4">
          {(hasPhotoChange || account.kyc?.hotelPhotos?.urls?.length > 0) && (
            <div className="bg-white rounded-2xl border border-gray-200 p-5">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                Photos de l'hôtel
                {hasPhotoChange && (
              ...
              <PhotoGrid
                urls={account.kyc?.hotelPhotos?.urls || []}
                label="Photos actuelles"
              />
              {account.kyc?.hotelPhotos?.uploadedAt && (
                <p className="text-xs text-gray-400 mt-2">
                  Mises à jour le {fmt(account.kyc.hotelPhotos.uploadedAt)}
                </p>
              )}
            </div>
          )}

          {/* Info change summary if only text fields changed */}
          {hasInfoChanges && !hasDocChanges && !hasPhotoChange && (
            <div className="bg-white rounded-2xl border border-gray-200 p-5">
              ...
              <p className="text-sm text-gray-600">
                L'opérateur a mis à jour des informations de son profil business.
                Vérifiez les informations actuelles à gauche et approuvez si elles sont correctes.
              </p>
              <div className="flex flex-wrap gap-2 mt-3">
                {changes
                  .filter(c => !['rccmDoc', 'nationalIdDoc', 'hotelPhotos'].includes(c))
                  .map(c => (
          ...
          {/* Approve / reject summary card */}
          <div className="bg-gray-50 rounded-2xl border border-gray-200 p-4">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
              Votre décision
            </p>
            <p className="text-sm text-gray-600 mb-4">
              Le compte reste <span className="font-semibold text-green-700">actif</span> et
              visible dans l'app pendant votre révision.
...
const PendingProfileReviewsTab = () => {
  ...
  const load = useCallback(async (p = 1, s = search) => {
    try {
      setLoading(true);
      setError('');
      const data = await hotelKycService.getPendingProfileReviews({
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-gray-900">
            Révisions de profil en attente
          </h2>
          <p className="text-sm text-gray-500 mt-0.5">
            {loading
              ? 'Chargement...'
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="flex items-center gap-1 text-xs text-gray-500">
                      <User className="h-3 w-3" /> {account.fullName}
                    </span>
                    <span className="flex items-center gap-1 text-xs text-gray-500">
                      <Mail className="h-3 w-3" /> {account.email}
                    </span>
                    <span className="flex items-center gap-1 text-xs text-gray-400">
                      <Clock className="h-3 w-3" /> {fmtRelative(account.pendingProfileReviewAt)}
                    </span>
                  </div>
                  {/* What changed chips */}
                  {changes.length > 0 && (
