...
const EDIT_FIELDS = [
  ...
  { key: "phoneNumber", label: "Téléphone perso", type: "text" },
  { key: "businessName", label: "Nom du business", type: "text" },
  { key: "businessAddress", label: "Adresse", type: "text" },
  { key: "hotelPhoneNumber", label: "Tél. hôtel", type: "text" },
...
// ── Action Modal (suspend/unsuspend/ban) ──────────────────────
...
const EditModal = ({ operator, isAdmin, onClose, onSave }) => {
  ...
  const [newPhotos, setNewPhotos] = useState([]); // File[] to upload
  const [removedUrls, setRemovedUrls] = useState([]); // existing URLs marked for removal
  ...
  // Photos still visible (not yet removed)
  ...
  const handleRemoveExisting = async (url) => {
    if (isAdmin) {
      ...
      // Salesperson: mark for removal, will be included in the request
      setRemovedUrls((p) => [...p, url]);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // For salesperson requests, pass removed URLs so admin can see what they want removed
        ...(removedUrls.length > 0 &&
          !isAdmin && { removedPhotos: removedUrls.join(",") }),
      };
        ...
        {existingUrl && !file && (
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-600 hover:bg-gray-100"
            >
              ↓
            </button>
          </div>
          ...
          <div>
            ...
            {!isAdmin && (
              <p className="text-xs text-amber-600 mt-0.5">
                ⚠️ Votre modification sera soumise à validation admin
              </p>
            )}
          </div>
          <button
        ...
        <div className="p-5 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {EDIT_FIELDS.map(({ key, label }) => (
                <div
                  key={key}
                  className={
                    key === "businessName" || key === "businessAddress"
                      ? "col-span-2"
                      : ""
                  }
                >
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <DocPicker
                label="RCCM"
                existingUrl={existingRccm}
                file={rccmFile}
                onFile={setRccmFile}
              />
              <DocPicker
                label="Carte nationale d'identité"
                existingUrl={existingNationalId}
                file={nationalIdFile}
                onFile={setNationalIdFile}
              />
              <DocPicker
                label="NIF (optionnel)"
                existingUrl={existingNif}
                file={nifFile}
                onFile={setNifFile}
              />
            </div>
          ...
          <div>
              ...
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">
                ...
                {newPhotos.length > 0 && (
                  <span className="ml-2 text-blue-600 normal-case font-normal">
                    +{newPhotos.length} à ajouter
                  </span>
                )}
                {removedUrls.length > 0 && (
                  <span className="ml-2 text-red-500 normal-case font-normal">
                    −{removedUrls.length} à supprimer
                  </span>
                )}
              </p>
              <span className="text-xs text-gray-400">
                {visiblePhotos.length + newPhotos.length} photo
                {visiblePhotos.length + newPhotos.length !== 1 ? "s" : ""}
              </span>
            </div>
            ...
            <div className="flex gap-2 flex-wrap mb-3">
                ...
                <div key={url} className="relative group">
                  ...
                  <div className="absolute inset-0 bg-black/50 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                    <button
                      ...
                      title="Télécharger"
                      className="p-1.5 bg-white/20 hover:bg-white/40 rounded-lg text-white text-xs"
                    >
                      ↓
                    </button>
                    <button
              ...
              {newPhotos.map((file, i) => (
                <div key={i} className="relative group">
                  <img
                    ...
                    alt={`New ${i + 1}`}
                    className="w-20 h-20 object-cover rounded-xl border-2 border-blue-400"
                  />
                  ...
                  {/* Blue "new" badge */}
                  <span className="absolute top-1 left-1 px-1 py-0.5 bg-blue-500 text-white text-xs rounded font-bold leading-none">
                    +
                  </span>
                </div>
              ))}
              ...
              {/* Add button */}
              <label className="w-20 h-20 flex flex-col items-center justify-center bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 hover:bg-gray-100 hover:border-gray-300 cursor-pointer transition-colors">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setNewPhotos((p) =>
                      [...p, ...files].slice(0, 10 - visiblePhotos.length),
                    );
                  }}
                />
              </label>
        ...
        <div className="flex items-center justify-between p-5 border-t border-gray-100">
          <p className="text-xs text-gray-400">
            {[
              ...
              newPhotos.length > 0 &&
                `${newPhotos.length} photo${newPhotos.length > 1 ? "s" : ""} à ajouter`,
              removedUrls.length > 0 &&
                `${removedUrls.length} photo${removedUrls.length > 1 ? "s" : ""} supprimée${removedUrls.length > 1 ? "s" : ""}`,
            ]
              .filter(Boolean)
  ...
  return (
    <div className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="relative h-36 bg-gray-200">
        ...
        <div
          className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-bold ${room.isActive ? "bg-green-500 text-white" : "bg-gray-400 text-white"}`}
        >
      ...
      <div className="p-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-1 text-xs text-gray-500">
            <Bed className="h-3 w-3" /> {room.maxGuests} pers. ·{" "}
            {room.totalRooms} ch.
          </div>
        </div>
        {room.nightlyRate && (
          <p className="text-sm font-semibold text-blue-600">
            ${room.nightlyRate.price}
...
const LocationCard = ({ location }) => {
  ...
  const [photoIdx, setPhotoIdx] = useState(0);
  const photos = [location.coverImage, ...(location.images || [])].filter(
    Boolean,
  );
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
      ...
      <div className="relative h-44 bg-gray-100">
              ...
              <>
                <div className="absolute inset-0 flex items-center justify-between px-2">
                  ...
                  <button
                    onClick={() =>
                      setPhotoIdx((i) =>
                        Math.min(uniquePhotos.length - 1, i + 1),
                      )
                    }
                    className="p-1 bg-black/40 rounded-full text-white hover:bg-black/60"
                  >
        ...
        <div
          className={`absolute top-2 left-2 px-2 py-0.5 rounded-full text-xs font-bold ${location.isActive ? "bg-green-500 text-white" : "bg-gray-400 text-white"}`}
        >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            ...
            <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
              ...
              {[location.commune, location.ville, location.province]
                .filter(Boolean)
                .join(", ")}
            </p>
          </div>
          <div className="text-right flex-shrink-0">
            ...
            <p className="text-xs text-gray-400">
              {location.activeRoomCount} chambre
              {location.activeRoomCount !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        ...
        {location.phoneNumber && (
          <a
            ...
            className="flex items-center gap-1.5 text-xs text-gray-500 mb-3 hover:text-blue-600"
          >
        ...
        {location.rooms?.length > 0 && (
          ...
          >
            <span>
              {location.rooms.length} type{location.rooms.length > 1 ? "s" : ""}{" "}
              de chambre{location.rooms.length > 1 ? "s" : ""}
            </span>
            {expanded ? (
...
const OperatorDetail = ({ operatorId, isAdmin, onBack, onRefresh }) => {
  ...
  const handleAction = async (reason) => {
    ...
    try {
      ...
      if (actionModal === "suspend")
        await hotelKycService.suspend(operatorId, reason);
      if (actionModal === "unsuspend")
        await hotelKycService.unsuspend(operatorId);
  ...
  const handleReviewEdit = async (requestId, action) => {
    const rejectionReason =
      action === "reject" ? prompt("Raison du rejet :") : undefined;
  ...
  if (!data)
    return (
      <div className="text-center py-16 text-gray-500">
        ...
        <button
          onClick={onBack}
          className="block mx-auto mt-4 text-blue-600 text-sm"
        >
          ← Retour
        </button>
      </div>
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {account.pendingProfileReview && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-orange-100 text-orange-700 text-xs font-bold rounded-full">
                <AlertTriangle className="h-3 w-3" /> Révision en attente
              </span>
            )}
            {pendingEdits.length > 0 && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-100 text-purple-700 text-xs font-bold rounded-full">
                <Edit className="h-3 w-3" /> {pendingEdits.length} modif. en
                attente
              </span>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
              ...
              {[
                ...
                {
                  label: "Tél. hôtel",
                  value: account.hotelPhoneNumber,
                  icon: Phone,
                },
                {
                  label: "Adresse",
                ...
                {
                  label: "Localisation",
                  value: [account.commune, account.city, account.province]
                    .filter(Boolean)
                    .join(", "),
                  icon: MapPin,
                },
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-3">
                  ...
                  <a
                    ...
                    className={`flex flex-col items-center p-3 border-2 border-dashed rounded-xl text-center transition-colors ${
                      doc?.url
                        ...
                        : "border-gray-200 bg-gray-50 opacity-40 pointer-events-none"
                    }`}
                  >
                    <FileText
                      className={`h-5 w-5 mb-1 ${doc?.url ? "text-blue-500" : "text-gray-400"}`}
                    />
              ...
              {/* Hotel photos from KYC */}
              {account.kyc?.hotelPhotos?.urls?.length > 0 && (
                <div>
                  <p className="text-xs text-gray-400 mb-2">
                    Photos hôtel ({account.kyc.hotelPhotos.urls.length})
                  </p>
                      ...
                      <div key={i} className="relative group">
                        ...
                        <div className="absolute inset-0 bg-black/50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="p-1 bg-white/20 hover:bg-white/40 rounded text-white text-xs"
                            title="Voir en grand"
                          >
              ...
              {account.kyc?.reviewedBy && (
                <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs text-gray-400">
                      {account.status === "kyc_rejected"
                        ? "Rejeté par"
                        : "Approuvé par"}
                    </p>
                    <p className="text-sm font-semibold text-gray-900">
                      {account.kyc.reviewedBy.name}
                    </p>
                    <p className="text-xs text-gray-400">
                      {account.kyc.reviewedBy.role} ·{" "}
                      {fmt(account.kyc.reviewedAt)}
                    </p>
                ...
                {isSuspended && account.suspendedBy && (
                  <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-xl border border-amber-100">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {account.suspendedBy.name}
                      </p>
                      <p className="text-xs text-gray-400">
                        {account.suspendedBy.role} · {fmt(account.suspendedAt)}
                      </p>
                ...
                {isBanned && account.bannedBy && (
                  <div className="flex items-start gap-3 p-3 bg-red-50 rounded-xl border border-red-100">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {account.bannedBy.name}
                      </p>
                      <p className="text-xs text-gray-400">
                        {account.bannedBy.role} · {fmt(account.bannedAt)}
                      </p>
                      {account.banReason && (
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: "Staff", value: account.staffCount },
                  {
                    ...
                    value: account.daysUntilExpiry || "—",
                  },
                  {
                    label: "Essai gratuit",
                    value: account.freeTrialEnabled
                      ? account.freeTrialUsed
                        ? "Utilisé"
                        : "Actif"
                      : "Non",
                  },
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-4">
            ...
            <div className="flex items-center gap-2">
              {account.registrationFeeWaived ? (
                <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-100 text-purple-700 text-sm font-bold rounded-xl">
                  ✅ Dispensé (promo: {account.promoCodeUsed})
                </span>
              ) : account.registrationFeePaid ? (
                <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-green-100 text-green-700 text-sm font-bold rounded-xl">
                  ✅ Payé — ${account.registrationFeeAmount}
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-red-100 text-red-700 text-sm font-bold rounded-xl">
                  ❌ Non payé — ${account.registrationFeeAmount}
                </span>
              )}
            </div>
          </div>
            ...
            {account.freeTrialEnabled ? (
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-600">
                  Début: {fmt(account.freeTrialStartedAt)}
                </p>
                <p className="text-sm text-gray-600">
                  Fin: {fmt(account.freeTrialEndsAt)}
                </p>
                <p className="text-sm font-bold text-gray-900">
                  {account.daysUntilExpiry} jours restants
                </p>
              </div>
            ) : account.subscription?.plan ? (
              <div className="space-y-2">
                <p className="font-semibold text-gray-900 capitalize">
                  {account.subscription.plan.replace("_", " ")}
                </p>
                <p className="text-sm text-gray-600">
                  Début: {fmt(account.subscription.startDate)}
                </p>
                <p className="text-sm text-gray-600">
                  Expiration: {fmt(account.subscription.endDate)}
                </p>
                <p className="text-sm text-gray-600">
                  Montant payé:{" "}
                  <span className="font-bold">
                    ${account.subscription.amountPaid}{" "}
      ...
      {activeTab === "history" && (
        <div className="space-y-4">
          {/* Pending edit requests — admin action needed */}
          {isAdmin && pendingEdits.length > 0 && (
            <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 space-y-3">
              <p className="text-sm font-bold text-purple-800">
                ⏳ {pendingEdits.length} modification
                {pendingEdits.length > 1 ? "s" : ""} en attente de validation
              </p>
              {pendingEdits.map((e) => (
                <div
                  key={e.id}
                  className="bg-white rounded-xl border border-purple-100 p-4"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900">
                        ...
                        <span className="text-gray-400 font-normal text-xs">
                          ({e.requestedBy?.salesId})
                        </span>
                      </p>
                      <p className="text-xs text-gray-400 mb-2">
                        {fmtFull(e.requestedAt)}
                      </p>
                      <div className="space-y-1">
                        {Object.entries(e.changes).map(([field, value]) => (
                          ...
                          >
                            <span className="text-gray-500 font-medium min-w-24">
                              {EDIT_FIELDS.find((f) => f.key === field)
                                ?.label || field}
                              :
                            </span>
                            <span className="text-red-500 line-through">
                              {e.previousValues?.[field] || "—"}
                            </span>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      <button
                        onClick={() => handleReviewEdit(e.id, "approve")}
                      ...
                      <button
                        onClick={() => handleReviewEdit(e.id, "reject")}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {e.reviewedBy && (
                        <p className="text-xs text-gray-400 mt-1">
                          {e.status === "approved" ? "Approuvé" : "Rejeté"} par{" "}
...
const HotelOperatorsTab = () => {
  ...
  const [selectedId, setSelectedId] = useState(null); // drill-in to detail
  ...
  const load = useCallback(
    async (p = 1, s = search, sf = statusFilter) => {
      try {
        setLoading(true);
        setError("");
        const data = await hotelKycService.getAll({
          page: p,
          limit: 20,
          search: s || undefined,
          status: sf || undefined,
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading
              ? "Chargement..."
              : `${total} opérateur${total !== 1 ? "s" : ""} au total`}
          </p>
        </div>
        <button
      ...
      {!loading && total > 0 && (
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div
              key={s.label}
              className={`border rounded-2xl p-3 text-center ${s.color}`}
            >
        ...
        <div className="flex flex-col items-center justify-center h-48 bg-white rounded-2xl border border-gray-200">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {search || statusFilter
              ...
              : "Les opérateurs apparaîtront ici."}
          </p>
        </div>
      ) : (
        <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                <div className="text-right">
                  <p className="text-xs text-gray-500">
                    {op.hotelsCount} hôtel{op.hotelsCount !== 1 ? "s" : ""}
                  </p>
                  {op.subscription?.plan && (
                    <p className="text-xs text-blue-600 font-medium">
                      {op.subscription.plan.replace("_", " ")}
                    </p>
                  )}
                </div>
