...
const ImageUploadField = ({ value, onChangeImage }) => {
  ...
  const handleFile = async (e) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { setError('Seuls les fichiers image sont autorisés'); return; }
    if (file.size > 3 * 1024 * 1024) { setError("L'image ne doit pas dépasser 3 Mo"); return; }
    const localUrl = URL.createObjectURL(file);
    setPreview(localUrl); setError('');
    try {
      setUploading(true);
      const fd = new FormData();
      fd.append('image', file);
      const res = await apiClient.post('/ads/upload-image', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      const url = res.data?.data?.imageUrl || res.data?.imageUrl;
      if (url) { setPreview(url); onChangeImage(url); }
  ...
  const clearImage = () => { setPreview(''); onChangeImage(''); if (fileRef.current) fileRef.current.value = ''; setError(''); };
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        <input
          ...
          value={preview}
          onChange={e => { setPreview(e.target.value); onChangeImage(e.target.value); setError(''); }}
        />
        <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
          className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700 disabled:opacity-50">
      ...
      {preview && (
        <div className="relative">
          <img src={preview} alt="Preview" className="max-h-32 rounded-lg border border-gray-200 object-contain"
      ...
      <p className="text-xs text-gray-400">Formats acceptés : JPG, PNG, WEBP · Taille max : 3 Mo · Recommandé : 430×64 px</p>
    </div>
  );
};
      ...
      <div className="relative w-full h-full flex items-center px-3 gap-3"
        ...
        <div className="relative z-10 flex items-center gap-3 w-full">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.7)' }}>{form.advertiser || ''}</p>
            <p className="text-white text-xs font-semibold truncate">{form.message || ''}</p>
          </div>
          {form.ctaValue && (
...
const AdFormModal = ({ ad, onClose, onSave }) => {
  ...
  const validateForm = () => {
    if (form.ctaType === 'call' || form.ctaType === 'whatsapp') {
      ...
      if (!phoneRegex.test(form.ctaValue.replace(/\s/g, ''))) {
        setError('Veuillez entrer un numéro de téléphone valide'); return false;
      }
    } else if (form.ctaType === 'link') {
      const urlRegex = /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/;
      if (!urlRegex.test(form.ctaValue)) { setError('Veuillez entrer une URL valide'); return false; }
    }
    return true;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const submitData = {
        advertiser: form.advertiser.trim(), logo: form.logo || null,
        message: form.message.trim(), ctaLabel: form.ctaLabel.trim(),
        ctaType: form.ctaType, ctaValue: form.ctaValue.trim(),
        startDate: form.startDate, endDate: form.endDate || null,
        priority: Number(form.priority), displayDurationSeconds: Number(form.displayDurationSeconds),
        isActive: form.isActive,
      };
      if (ad?._id) await apiClient.patch(`/ads/${ad._id}`, submitData);
      else         await apiClient.post('/ads', submitData);
      onSave(); onClose();
    } catch (e) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">{ad ? 'Modifier la publicité' : 'Nouvelle publicité'}</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <input className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="Ex: Vodacom, MTN..." value={form.advertiser} onChange={e => set('advertiser', e.target.value)} />
          </div>

          <ImageUploadField value={form.logo} onChangeImage={v => set('logo', v)} />

          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1.5">
              Message défilant <span className="text-gray-400 font-normal">({form.message.length}/200)</span>
            </label>
            <textarea className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
              rows={2} maxLength={200} placeholder="Le message qui défile sur la bannière..."
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            ...
            <div>
              ...
              <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={form.ctaType} onChange={e => set('ctaType', e.target.value)}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input className="w-full border border-gray-200 rounded-lg pl-9 pr-4 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder={form.ctaType === 'call' || form.ctaType === 'whatsapp' ? '+243...' : form.ctaType === 'link' ? 'https://...' : 'Ex: HotelDetail'}
                  value={form.ctaValue} onChange={e => set('ctaValue', e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
              ...
              <input type="range" min={5} max={30} step={1} className="w-full accent-blue-600"
                value={form.displayDurationSeconds} onChange={e => set('displayDurationSeconds', Number(e.target.value))} />
            ...
            <div>
              ...
              <input type="range" min={1} max={10} step={1} className="w-full accent-blue-600"
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-gray-500 mt-0.5 truncate" title={ad.ctaValue}>
...
const AdsTab = ({ hasFullAccess }) => {
  ...
  const fetchAds = async () => {
    try {
      ...
      if (e.response?.status === 403) alert('Accès non autorisé.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggle = async (ad) => {
    try { await apiClient.patch(`/ads/${ad._id}/toggle`); await fetchAds(); }
    catch { alert('Erreur lors de la modification du statut'); }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteConfirm(null); await fetchAds();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {hasFullAccess && (
          <button onClick={() => { setEditingAd(null); setShowModal(true); }}
            className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 shadow-sm">
      ...
      <div className="flex gap-2 border-b border-gray-200">
        {[
          ...
          <button key={tab.key} onClick={() => setFilter(tab.key)}
            className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${filter === tab.key ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            {tab.label}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredAds.map(ad => (
            <AdCard key={ad._id} ad={ad}
              onEdit={(ad) => { setEditingAd(ad); setShowModal(true); }}
      ...
      {showModal && (
        <AdFormModal ad={editingAd}
          onClose={() => { setShowModal(false); setEditingAd(null); }}
          onSave={() => { setShowModal(false); setEditingAd(null); fetchAds(); }}
        />
      )}
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)}
                className="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl">Annuler</button>
              <button onClick={handleDelete} disabled={deleting}
                className="flex-1 px-4 py-2.5 text-sm font-semibold text-white bg-red-500 hover:bg-red-600 rounded-xl disabled:opacity-60">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${
          ...
          {hasFullAccess ? <Shield className="h-6 w-6 text-white" /> : <Megaphone className="h-6 w-6 text-white" />}
        </div>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Publicités & Notifications</h1>
          <p className="text-sm text-gray-500">
            Bannières publicitaires et notifications push
      ...
      <div className="flex gap-1 p-1 bg-gray-100 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            tab === 'ads' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
      ...
      {/* Tab content */}
      {tab === 'ads'           && <AdsTab hasFullAccess={hasFullAccess} />}
